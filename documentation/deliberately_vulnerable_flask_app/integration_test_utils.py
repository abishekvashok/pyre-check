...
def normalized_json_dump(
    ...
    """
    Returns a normalised JSON string from results keeping only essential items. Filters
    issues down to issues that have the code we intend to test for if filter_issues is
    true, and removes location information if strip_location is set to True.
    """
    ...
    if filter_issues:
        # Filter down to only issues that have the code that we intended to test for.
        # This prevents the introduction of new rules or false positives from
...
def run_pysa_analysis(
    ...
    raw_results.json file that contains the output. Creates position_invariant_result.json
    that contains position information to compare using diff with
    position_invariant_result.actual before exiting  if there is a mismatch between the specified
    and detected issues.
    """
    ...
    try:
        pysa_results = subprocess.check_output(command, text=True)
    ...
    if normalized_pysa_results != normalized_expected_results:
        ...
        result = subprocess.run(
            [
                "diff",
                "-u",
                expected_invariant_results_path.name,
                actual_invariant_results_path.name,
            ],
            text=True,
